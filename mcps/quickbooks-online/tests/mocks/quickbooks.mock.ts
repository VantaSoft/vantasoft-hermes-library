...
export const mockQuickbooksClient = {
  authenticate: jest
    .fn<() => Promise<typeof mockQuickBooksInstance>>()
    .mockResolvedValue(mockQuickBooksInstance),
  getQuickbooks: jest
    .fn<() => typeof mockQuickBooksInstance>()
    .mockReturnValue(mockQuickBooksInstance),
...
export const mockQuickbooksClientClass = {
  getInstance: jest
    .fn<() => Promise<typeof mockQuickBooksInstance>>()
    .mockResolvedValue(mockQuickBooksInstance),
  getAuthCredentials: jest
    .fn<() => Promise<typeof mockAuthCredentials>>()
    .mockResolvedValue(mockAuthCredentials),
};
...
export function mockFindCallback<T>(entities: T[]) {
  return (...args: unknown[]) => {
    const callback = args[args.length - 1] as (
      err: null,
      result: { QueryResponse: Record<string, T[]> },
    ) => void;
...
export function resetAllMocks() {
  ...
  (mockQuickbooksClient.getQuickbooks as any).mockReturnValue(
    mockQuickBooksInstance,
  );
  (mockQuickbooksClient.authenticate as any).mockResolvedValue(
    mockQuickBooksInstance,
  );
  mockQuickbooksClientClass.getInstance.mockReset();
  (mockQuickbooksClientClass.getInstance as any).mockResolvedValue(
    mockQuickBooksInstance,
  );
