...
const { createQuickbooksSalesReceipt } = await import(
  "../../../src/handlers/create-quickbooks-sales-receipt.handler"
);
const { getQuickbooksSalesReceipt } = await import(
  "../../../src/handlers/get-quickbooks-sales-receipt.handler"
);
const { updateQuickbooksSalesReceipt } = await import(
  "../../../src/handlers/update-quickbooks-sales-receipt.handler"
);
const { deleteQuickbooksSalesReceipt } = await import(
  "../../../src/handlers/delete-quickbooks-sales-receipt.handler"
);
const { searchQuickbooksSalesReceipts } = await import(
  "../../../src/handlers/search-quickbooks-sales-receipts.handler"
);
...
describe("SalesReceipt Handlers", () => {
  ...
  describe("createQuickbooksSalesReceipt", () => {
    it("should create a sales receipt successfully", async () => {
      ...
      mockQuickBooksInstance.createSalesReceipt.mockImplementation(
        (payload: any, cb: any) => cb(null, mockReceipt),
      );
      ...
      const payload = (
        mockQuickBooksInstance.createSalesReceipt.mock.calls[0] as any
      )[0];
    ...
    it("should pass per-line TaxCodeRef and GlobalTaxCalculation to QuickBooks", async () => {
      mockQuickBooksInstance.createSalesReceipt.mockImplementation(
        (payload: any, cb: any) => cb(null, { Id: "123" }),
      );
    ...
    it("should handle API errors", async () => {
      mockQuickBooksInstance.createSalesReceipt.mockImplementation(
        (payload: any, cb: any) => cb(new Error("Validation error"), null),
      );
  ...
  describe("getQuickbooksSalesReceipt", () => {
    it("should get a sales receipt by ID", async () => {
      const mockReceipt = { Id: "123", TotalAmt: 250 };
      mockQuickBooksInstance.getSalesReceipt.mockImplementation(
        (_id: any, cb: any) => cb(null, mockReceipt),
      );
  ...
  describe("updateQuickbooksSalesReceipt", () => {
    it("should update a sales receipt", async () => {
      ...
      mockQuickBooksInstance.updateSalesReceipt.mockImplementation(
  ...
  describe("deleteQuickbooksSalesReceipt", () => {
    it("should void a sales receipt", async () => {
      mockQuickBooksInstance.deleteSalesReceipt.mockImplementation(
        (payload: any, cb: any) => cb(null, {}),
      );

      const result = await deleteQuickbooksSalesReceipt({
        id: "123",
        sync_token: "0",
      });
  ...
  describe("searchQuickbooksSalesReceipts", () => {
    it("should search sales receipts", async () => {
      ...
      mockQuickBooksInstance.findSalesReceipts.mockImplementation(
        (criteria: any, cb: any) =>
