...
const { searchQuickbooksPurchases } = await import(
  "../../../src/handlers/search-quickbooks-purchases.handler"
);
const { searchQuickbooksEmployees } = await import(
  "../../../src/handlers/search-quickbooks-employees.handler"
);
...
describe("search_purchases – Fixes #14", () => {
  ...
  it("should return empty array when no purchases match", async () => {
    (mockQuickBooksInstance.findPurchases as jest.Mock).mockImplementation(
      (_criteria: any, cb: any) =>
        cb(null, { QueryResponse: {}, time: "2026-01-01" }),
    );
