...
function mockHttpsUploadNetworkError(err: Error) {
  ...
  (mockHttpsRequest as any).mockImplementation(
    (_options: unknown, _callback: unknown) => mockReq,
  );
}

// Dynamic imports after mock setup
const { getQuickbooksCompanyInfo } = await import(
  "../../../src/handlers/get-quickbooks-company-info.handler"
);
const { updateQuickbooksCompanyInfo } = await import(
  "../../../src/handlers/update-quickbooks-company-info.handler"
);
const { createQuickbooksAttachable } = await import(
  "../../../src/handlers/create-quickbooks-attachable.handler"
);
const { getQuickbooksAttachable } = await import(
  "../../../src/handlers/get-quickbooks-attachable.handler"
);
const { updateQuickbooksAttachable } = await import(
  "../../../src/handlers/update-quickbooks-attachable.handler"
);
const { deleteQuickbooksAttachable } = await import(
  "../../../src/handlers/delete-quickbooks-attachable.handler"
);
const { searchQuickbooksAttachables } = await import(
  "../../../src/handlers/search-quickbooks-attachables.handler"
);
...
describe("Company and Attachable Handlers", () => {
  ...
  describe("getQuickbooksCompanyInfo", () => {
    it("should get company info", async () => {
      ...
      mockQuickBooksInstance.getCompanyInfo.mockImplementation(
        (_id: any, cb: any) => cb(null, mockInfo),
      );
    ...
    it("should handle errors", async () => {
      mockQuickBooksInstance.getCompanyInfo.mockImplementation(
        (_id: any, cb: any) => cb(new Error("Not authorized"), null),
      );
  ...
  describe("updateQuickbooksCompanyInfo", () => {
    it("should update company info", async () => {
      ...
      mockQuickBooksInstance.updateCompanyInfo.mockImplementation(
        (payload: any, cb: any) => cb(null, mockUpdated),
      );
  ...
  describe("createQuickbooksAttachable", () => {
    it("should create an attachable", async () => {
      ...
      mockQuickBooksInstance.createAttachable.mockImplementation(
        (payload: any, cb: any) => cb(null, mockAttachable),
      );
    ...
    it("should propagate IncludeOnSend through attachable_ref", async () => {
      ...
      expect(capturedPayload.AttachableRef[0].EntityRef).toEqual({
        type: "Invoice",
        value: "42",
      });
    });
  ...
  describe("createQuickbooksAttachable — binary upload", () => {
    it("should upload PDF when base64_content is provided", async () => {
      const uploadResponse = {
        AttachableResponse: [
          {
            Attachable: { Id: "99", FileName: "invoice.pdf" },
            responseStatus: "200",
          },
        ],
      };
      ...
      const [[reqOptions]] = (mockHttpsRequest as jest.Mock).mock
        .calls as any[][];
    ...
    it("should select production host when isSandbox is false", async () => {
      const { mockAuthCredentials } = await import(
        "../../mocks/quickbooks.mock"
      );
    ...
    it("should reject base64_content exceeding 100 MB before decoding", async () => {
      ...
      const overLimitBase64Length = Math.ceil(
        ((100 * 1024 * 1024 + 1) * 4) / 3,
      );
  ...
  describe("getQuickbooksAttachable", () => {
    it("should get an attachable by ID", async () => {
      const mockAttachable = { Id: "1", FileName: "test.pdf" };
      mockQuickBooksInstance.getAttachable.mockImplementation(
        (_id: any, cb: any) => cb(null, mockAttachable),
      );
  ...
  describe("updateQuickbooksAttachable", () => {
    it("should update an attachable", async () => {
      mockQuickBooksInstance.updateAttachable.mockImplementation(
        (payload: any, cb: any) => cb(null, {}),
      );
  ...
  describe("deleteQuickbooksAttachable", () => {
    it("should delete an attachable", async () => {
      mockQuickBooksInstance.deleteAttachable.mockImplementation(
        (payload: any, cb: any) => cb(null, {}),
      );

      const result = await deleteQuickbooksAttachable({
        id: "1",
        sync_token: "0",
      });
  ...
  describe("searchQuickbooksAttachables", () => {
    it("should search attachables", async () => {
      ...
      mockQuickBooksInstance.findAttachables.mockImplementation(
        (criteria: any, cb: any) =>
