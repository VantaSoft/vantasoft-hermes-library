...
const { searchQuickbooksBudgets } = await import(
  "../../../src/handlers/search-quickbooks-budgets.handler"
);
...
describe("Budget Handlers", () => {
  ...
  describe("searchQuickbooksBudgets", () => {
      ...
      const mockBudgets = [
        {
          ...
          BudgetDetail: [
            {
              AccountRef: { value: "80", name: "40000 Revenue" },
              Amount: 100000,
              BudgetDate: "2025-07-01",
            },
          ],
        },
      ];
      mockQuickBooksInstance.findBudgets.mockImplementation(
    ...
    it("should propagate API errors", async () => {
      mockQuickBooksInstance.findBudgets.mockImplementation(
        (_criteria: any, cb: any) => cb(new Error("Query failed"), null),
      );
