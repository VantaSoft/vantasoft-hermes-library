...
export async function searchQuickbooksPaymentMethods(
  data: SearchPaymentMethodsInput,
  ...
  try {
    ...
    return new Promise((resolve) => {
      (quickbooks as any).findPaymentMethods(
        criteria,
        (err: any, result: any) => {
          if (err)
            ...
            resolve({
              result: result?.QueryResponse?.PaymentMethod || [],
              isError: false,
              error: null,
            });
        },
