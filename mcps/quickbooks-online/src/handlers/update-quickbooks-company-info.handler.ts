...
export async function updateQuickbooksCompanyInfo(
  ...
  try {
    ...
    if (data.company_addr) {
      ...
      if (data.company_addr.country_sub_division_code)
        payload.CompanyAddr.CountrySubDivisionCode =
          data.company_addr.country_sub_division_code;
    ...
    return new Promise((resolve) => {
      (quickbooks as any).updateCompanyInfo(
        payload,
        (err: any, updated: any) => {
          if (err)
