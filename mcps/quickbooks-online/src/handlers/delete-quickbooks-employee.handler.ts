 ...
 */
export async function deleteQuickbooksEmployee(
  idOrEntity: any,
    ...
    return new Promise((resolve) => {
      ...
      const getEntity = (cb: (employee: any) => void) => {
        if (typeof idOrEntity === "object" && idOrEntity?.Id) {
          ...
          (quickbooks as any).getEmployee(idOrEntity, (e: any, emp: any) =>
            cb(emp),
          );
        }
      };
      ...
      getEntity((employeeEntity) => {
        if (!employeeEntity || !employeeEntity.Id) {
          resolve({
            ...
            isError: true,
            error: formatError(
              "Unable to retrieve employee for inactive update",
            ),
          });
          return;
        }
        ...
        (quickbooks as any).updateEmployee(
          inactiveEntity,
          (err: any, resp: any) => {
            if (err) {
