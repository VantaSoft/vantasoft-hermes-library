...
export async function deleteQuickbooksCustomer(
  ...
  try {
    ...
    return new Promise((resolve) => {
      if (hasDelete) {
        (quickbooks as any).deleteCustomer(
          idOrEntity,
      ...
      function fallbackInactive(originalError?: any) {
        ...
        const getEntity = (cb: (cust: any) => void) => {
          if (typeof idOrEntity === "object" && idOrEntity?.Id) {
            ...
            (quickbooks as any).getCustomer(idOrEntity, (e: any, cust: any) =>
              cb(cust),
            );
          }
        };
        ...
        getEntity((customerEntity) => {
          if (!customerEntity || !customerEntity.Id) {
            resolve({
              ...
              error: formatError(
                originalError ||
                  "Unable to retrieve customer for inactive update",
              ),
            });
            return;
          }
          ...
          (quickbooks as any).updateCustomer(
            inactiveEntity,
            (err: any, resp: any) => {
              if (err) {
                resolve({
