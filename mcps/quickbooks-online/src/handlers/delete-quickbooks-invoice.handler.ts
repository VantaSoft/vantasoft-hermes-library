...
export async function deleteQuickbooksInvoice(
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      if (hasDelete) {
        (quickbooks as any).deleteInvoice(
          idOrEntity,
      ...
      function voidInvoice(originalError?: any) {
        ...
        const getEntity = (cb: (invoice: any) => void) => {
          if (typeof idOrEntity === "object" && idOrEntity?.Id) {
            ...
            (quickbooks as any).getInvoice(idOrEntity, (e: any, inv: any) =>
              cb(inv),
            );
          }
        };
        ...
        getEntity((invoiceEntity) => {
          if (!invoiceEntity || !invoiceEntity.Id) {
            resolve({
              ...
              error: formatError(
                originalError ||
                  "Unable to retrieve invoice for void operation",
              ),
            });
            return;
          }
