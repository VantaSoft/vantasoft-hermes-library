...
export function isToolDisabled(toolName: string): boolean {
  ...
  // Accounting mutations are intentionally absent unless the operator opts in.
  // This prevents a newly installed server from exposing money-moving tools by
  // accident. Category-specific disable flags remain available after opt-in.
  if (process.env.QUICKBOOKS_ENABLE_MUTATIONS !== "true") return true;
 ...
 * Mutations require QUICKBOOKS_ENABLE_MUTATIONS=true. Category-specific
 * disable variables can then suppress write, update, or delete tools.
 */
