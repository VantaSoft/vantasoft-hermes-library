...
const toolDescription =
  "Search for payment terms in QuickBooks Online with optional filters.";
...
const toolHandler = async ({ params }: any) => {
  ...
  return {
    content: [
      { type: "text" as const, text: `Found ${response.result.length} terms:` },
      { type: "text" as const, text: JSON.stringify(response.result, null, 2) },
    ],
  };
};

export const SearchTermsTool: ToolDefinition<typeof toolSchema> = {
  name: toolName,
