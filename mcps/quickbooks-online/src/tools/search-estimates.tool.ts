...
export const SearchEstimatesTool: ToolDefinition<typeof toolSchema> = {
  ...
  schema: toolSchema,
  handler: async (args) => {
    const { criteria = [], ...options } = (args.params ?? {}) as z.infer<
      typeof toolSchema
    >;
    ...
    if (response.isError) {
      return {
        content: [
          {
            type: "text" as const,
            text: `Error searching estimates: ${response.error}`,
          },
        ],
      };
    }
    return {
      content: [
        {
          type: "text" as const,
          text: Array.isArray(response.result)
            ? `Found ${response.result.length} estimates:`
