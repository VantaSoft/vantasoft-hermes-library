...
const toolSchema = z.object({
  ...
  item_ref: z
    ...
    .optional()
    .describe(
      "Service Item ID (ItemRef) — the QBO Item to associate with this time entry",
    ),
});
...
const toolHandler = async ({ params }: any) => {
  ...
  return {
    content: [
      { type: "text" as const, text: `Time activity updated:` },
      { type: "text" as const, text: JSON.stringify(response.result, null, 2) },
    ],
  };
};

export const UpdateTimeActivityTool: ToolDefinition<typeof toolSchema> = {
  name: toolName,
