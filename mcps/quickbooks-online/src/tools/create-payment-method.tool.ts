...
const toolDescription =
  "Create a new payment method in QuickBooks Online (e.g., Cash, Check, Credit Card).";
...
const toolHandler = async ({ params }: any) => {
  ...
  return {
    content: [
      { type: "text" as const, text: `Payment method created:` },
      { type: "text" as const, text: JSON.stringify(response.result, null, 2) },
    ],
  };
};

export const CreatePaymentMethodTool: ToolDefinition<typeof toolSchema> = {
  name: toolName,
