...
const toolDescription =
  "Create a payment (receive money from customer) in QuickBooks Online.";
...
const toolSchema = z.object({
  ...
  deposit_to_account_ref: z
    .string()
    .optional()
    .describe("Account to deposit to"),
  ...
  line: z
    .array(lineSchema)
    .optional()
    .describe("Line items linking to invoices"),
});
  ...
  if (response.isError) {
    return {
      content: [
        {
          type: "text" as const,
          text: `Error creating payment: ${response.error}`,
        },
      ],
    };
  }
