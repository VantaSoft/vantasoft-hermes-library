...
const toolDescription =
  "Create a new class in QuickBooks Online for categorizing transactions.";
...
const toolHandler = async ({ params }: any) => {
  ...
  return {
    content: [
      { type: "text" as const, text: `Class created:` },
      { type: "text" as const, text: JSON.stringify(response.result, null, 2) },
    ],
  };
};

export const CreateClassTool: ToolDefinition<typeof toolSchema> = {
  name: toolName,
