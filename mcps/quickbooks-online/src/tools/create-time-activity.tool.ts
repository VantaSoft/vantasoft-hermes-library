...
const toolDescription =
  "Create a time activity (time tracking entry) in QuickBooks Online.";
const toolSchema = z.object({
  name_of: z
    .enum(["Vendor", "Employee"])
    .describe("Whether time is for vendor or employee"),
  vendor_ref: z
    .string()
    .optional()
    .describe("Vendor ID (if name_of is Vendor)"),
  employee_ref: z
    .string()
    .optional()
    .describe("Employee ID (if name_of is Employee)"),
...
const toolHandler = async ({ params }: any) => {
  ...
  return {
    content: [
      { type: "text" as const, text: `Time activity created:` },
      { type: "text" as const, text: JSON.stringify(response.result, null, 2) },
    ],
  };
};

export const CreateTimeActivityTool: ToolDefinition<typeof toolSchema> = {
  name: toolName,
