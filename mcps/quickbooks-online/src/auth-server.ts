 ...
 * 3. Handle the callback and save tokens to the profile credential file
 * 4. Close automatically when complete
 *
 * Prerequisites:
 * • QUICKBOOKS_CLIENT_ID and QUICKBOOKS_CLIENT_SECRET must be set in the profile credential file
...
async function main() {
  ...
  try {
    ...
    console.log("Tokens have been saved to the profile credential file.");
    ...
    console.error("\nPlease check:");
    console.error(
      "1. QUICKBOOKS_CLIENT_ID is set correctly in the profile credential file",
    );
    console.error(
      "2. QUICKBOOKS_CLIENT_SECRET is set correctly in the profile credential file",
    );
    console.error(
