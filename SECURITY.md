...
• Telvana credentials belong in `HERMES_HOME/mcp-tokens/telvana-admin/credentials.json`, with a separate least-privilege workspace API key per environment.
• The Telvana Admin MCP requires explicit environment and actor selection, hides mutations behind both process and credential opt-ins, requires a separate production opt-in, writes metadata-only audit records, and exposes no destructive, infrastructure, database, deployment, or credential-management tools.
